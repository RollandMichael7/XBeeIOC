...
class XBeeLTN(XBeeDevice):
    ...
    def __init__(self, addr):
        ...
        print "Product type: %s" %self.product_type
        print "SensorLT type: %s" %XBeeSensorLTAdapter
        # michael rolland - add support for LTH adapter
        if self.product_type != XBeeSensorLTAdapter and self.product_type != XBeeSensorLTHAdapter:
            #raise ValueError, "Adapter is not a %s" % (GetXBeeProductName(XBeeSensorLTAdapter))
            raise ValueError, "Adapter is a %s" % (GetXBeeProductName(self.product_type))
