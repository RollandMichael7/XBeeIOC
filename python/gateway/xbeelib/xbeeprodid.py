...
def GetXBeeDeviceType(address):
  ...
  for i in xrange(0, 5):
    try:
			device_type, product_type = struct.unpack('>HH', xbee.ddo_get_param(address, 'DD'))
    except:
			continue
    else:
  ...
  for i in xrange(0, 5):
		try:
			version = struct.unpack('=H', xbee.ddo_get_param(address, 'VR'))[0]
			hardware = struct.unpack('=H', xbee.ddo_get_param(address, 'HV'))[0]			
		except:
			continue
		else:
...
def GetXBeeDeviceNodeIdentifier(address):
	...
	for i in range(0, 10):
		try:
			node_name = xbee.ddo_get_param(address, 'NI')
		except:
			continue
		else:
			return node_name
